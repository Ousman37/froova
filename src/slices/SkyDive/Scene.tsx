...
type FlavorType = (typeof allowedFlavors)[number];
  ...
  return (
    <group ref={groupRef}>
      ...
      <group rotation={[0, 0, 0.5]}>
        <FloatingCan
          ref={canRef}
          flavor={flavor?.trim() as FlavorType}
          rotationIntensity={0}
          floatIntensity={3}
          floatSpeed={3}
        >
      ...
      {/* Text */}
      <group ref={wordsRef}>
        {sentence && <ThreeText sentence={sentence} color="#34D399" />}
      </group>

      {/* Lights */}
      <ambientLight intensity={2} color="#9f1280" />
      <Environment files="/hdr/field.hdr" environmentIntensity={1.5} />
    </group>
  );
}

// Text Component for displaying the sentence
function ThreeText({
  sentence,
  color = "white",
}: {
